    ...
    async (choice: string) => {
      ...
      switch (choice) {
        ...
        case "7":
          ...
          console.log(
            gradient(["red", "purple"])(
              "Link: https://discord.gg/5vd3jftEJm"
            )
          );
          awaitenter(client);
          break;
...
export function menutext(client: Client) {
  creatorname();
  const title = langat !== "en" 
    ? gradient(["#2180db", "#75a1cb", "#78a5d3"])("[0] AngelordX Developer\n") 
    : null;
  console.log((title ? title : '') + gradient(["#ff4500", "#ffa500", "#ff6347"])(t("menuText")));
  choiceinit(client);
}
      ...
      setTimeout(async () => {
        ...
        if (cloneOption === 3) {
          const template = await newGuild.createTemplate(
            `${guild.name}`,
            `By AngelordX (https://discord.gg/5vd3jftEJm)`
          );
